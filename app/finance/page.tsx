    ...
    <div className="flex items-start gap-2">
      {message.role === "assistant" && (
        <Avatar className="w-8 h-8 border">
          <AvatarImage src="/hum-logo.svg" alt="AI Assistant Avatar" />
...
export default function AIChat() {
  ...
  const [promptCount, setPromptCount] = useState<number | null>(null); // Change to null initially
  const PROMPT_LIMIT = 50;

  const [isLoadingCount, setIsLoadingCount] = useState(true);

  // Load prompt count from Firestore when component mounts
  useEffect(() => {
    const loadPromptCount = async () => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          await setDoc(userDocRef, { promptCount: 0 });
          setPromptCount(0);
        }
      } catch (error) {
        ...
        toast({
          title: "Error",
          description: "Failed to load prompt count",
          variant: "destructive",
        });
      } finally {
  ...
  const isLimitReached = promptCount !== null && promptCount >= PROMPT_LIMIT;

  // Update Firestore whenever promptCount changes
  const updatePromptCount = async (newCount: number) => {
    if (!user?.uid) return;

    try {
      const userDocRef = doc(db, "users", user.uid);
      await setDoc(userDocRef, { promptCount: newCount }, { merge: true });
    } catch (error) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const newPromptCount = (promptCount ?? 0) + 1;
    setPromptCount(newPromptCount);
    await updatePromptCount(newPromptCount);
                ...
                <div className="flex flex-col items-center justify-center h-full animate-fade-in-up max-w-[95%] mx-auto">
                  ...
                  <div className="space-y-4 text-base">
                    <Alert className="mb-4">
                      <AlertDescription>
                        Beta access: You have {PROMPT_LIMIT} prompts available
                      </AlertDescription>
                    </Alert>
                ...
                <div className="flex flex-col space-y-2">
                  {isLimitReached && (
                    <Alert variant="destructive" className="mb-2">
                      <AlertDescription>
                        You have reached the beta limit of {PROMPT_LIMIT}{" "}
                        prompts. Thank you for testing!
                      </AlertDescription>
                    </Alert>
                  )}
                  {currentUpload && (
                  ...
                  <div className="flex items-end space-x-2">
                    <div className="flex-1 relative">
                      <Button
                        ...
                        disabled={isLoading || isUploading || isLimitReached}
                  ...
                  <div className="flex flex-col items-center justify-center gap-4 -translate-y-8">
                    ...
                    <div className="space-y-2">
                      ...
                      <CardDescription className="text-base">
                        Charts and detailed analysis will appear here as you
                        chat
                      </CardDescription>
